...
class uLogin
{
	...
	private static function ValidateUsername($str)
	{
		...
		if (UL_ALPHANUMERIC_USER)
		{
			// Remove valid characters, so that they won't be
			// checked by ctype_alnum
			$valid = UL_USERNAME_EXTRA_CHARS;
			$tmp = $str;
			for ($i = 0; $i < strlen($valid); ++$i)
				$tmp = str_replace($valid[$i], '', $tmp);
	...
	private function AuthSuccess($uid, $username)
	{
		...
		// Log authentication
		ulLog::Log('auth-success', $username, $_SERVER['REMOTE_ADDR']);
	...
	private function AuthFail($uid, $username)
	{
		...
		ulLog::Log('auth-fail', $username, $_SERVER['REMOTE_ADDR']);
	...
	private function BlockCheck($uid)
	{
		...
		if (UL_BF_IP_LOCKOUT > 0)
		{
			$block_expires = ulIpBlocker::IpBlocked($_SERVER['REMOTE_ADDR']);
	...
	// Given a uid and a password, this function returns true,
	...
	{
		$username = $this->Username($uid);
		ulLog::Log('logout', $username, $_SERVER['REMOTE_ADDR']);
  ...
  // backend is supposed to simultanously contain login and profile information (eg. LDAP.)
	public function CreateUser($username, $password, $profile=NULL)
	{
		...
		ulLog::Log('create login', $username, $_SERVER['REMOTE_ADDR']);
	...
	public function DeleteUser($uid)
	{
		...
		if ($ret === true)
			ulLog::Log('delete login', $username, $_SERVER['REMOTE_ADDR']);
		return $ret === true;
	}
